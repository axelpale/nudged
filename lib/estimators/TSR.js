...
module.exports = function (domain, range) {
  ...
  // Estimates
  const ahat = (N * (ac + bd) - a1 * c1 - b1 * d1) / den
  const bhat = (N * (ad - bc) + b1 * c1 - a1 * d1) / den
  const xhat = (-a1 * (ac + bd) + b1 * (ad - bc) + a2 * c1 + b2 * c1) / den
  const yhat = (-b1 * (ac + bd) - a1 * (ad - bc) + a2 * d1 + b2 * d1) / den
